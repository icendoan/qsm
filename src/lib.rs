...
impl Connection
{
    fn tcp(addr: &str, port: Port, user: Option<&str>, pass: Option<&str>) -> R<Connection>
    {
    ...
    fn unix(path: &str, user: Option<&str>, pass: Option<&str>) -> R<Connection>
    {
    ...
    fn resp(&mut self) -> R<KOwned>
    {
        ...
        unsafe {
            let kbuf: *const K = kbindings::ktn(kbindings::KG as i32, len as i64);
            if kbuf.is_null()
            {
    ...
    fn query(&mut self, query: String) -> R<()>
    {
        unsafe {
        ...
        fn settings(txt: &str) -> R<Action>
        {
            ...
            for kv in txt.split(';')
            {
                match split_key_value_pair(kv, "=")
                {
                    ...
                    Ok(("msgtype", m)) => b.msgtype(match m
                    {
                        "pretty" | "q" => Task::Q,
                        "plot" => Task::Plot,
                        "raw" | "k" => Task::K,
...
fn action(a: Action, servers: &mut HashMap<String, Server>, snippets: &mut HashMap<String, String>, settings: &mut Setting) -> R<bool>
{
    ...
    {
        name.and_then(|n| snippets.get(n)).map(|x| x.as_ref()).ok_or(Error::WithMsg("No such snippet."))
    }
...
:r <query> - Sends this query without any preprocessing.
:plot <query> - Plots the value of this query with gnuplot.
:pretty <query> - Enforces pretty printing for this query.
